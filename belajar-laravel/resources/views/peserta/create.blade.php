...
<div class="modern-card" style="max-width: 600px; margin: 0 auto;">
    <div class="d-flex align-items-center mb-4 border-bottom pb-3">
        ...
        <h2 class="page-title mb-0" style="font-weight: 700; color: #1e293b; font-size: 20px;">Tambah Data Peserta</h2>
    </div>
    ...
    <form action="{{ route('peserta.store') }}" method="post">
        ...
        <div class="mb-4">
            ...
            <input type="text" class="modern-input" id="name" name="name" value="{{ old('name') }}" placeholder="Masukkan nama" required>
        </div>
        <div class="mb-4">
            <label for="email" class="modern-label">Email</label>
            <input type="email" class="modern-input" id="email" name="email" value="{{ old('email') }}" placeholder="Masukkan email" required>
        </div>
        <div class="mb-4">
            <label for="age" class="modern-label">Umur</label>
            <input type="number" class="modern-input" id="age" name="age" value="{{ old('age') }}" placeholder="Masukkan umur" min="18" max="65" required>
        </div>
        <div class="mb-4">
            <label for="address" class="modern-label">Alamat</label>
            <input type="text" class="modern-input" id="address" name="address" value="{{ old('address') }}" placeholder="Masukkan alamat lengkap" required>
        </div>
        
        <div class="mt-5 pt-3 border-top d-flex justify-content-end">
            <button type="submit" class="btn-modern btn-primary px-5" style="width: 100%;">
                <i class="bi bi-check2-circle me-1" style="font-size: 18px;"></i> Simpan Data
            </button>
        </div>
    </form>
</div>
